...
class BlackListController
{
    ...
    private function mainView($content, $allow_edit=0)
    {
        return $this->view->renderMainView($content, $allow_edit, $_SESSION['user_id']);
    }
    ...
    public function toDocumentation(){
        $content = $this->view->renderToDocumentationView();
        echo $this->mainView($content, $_SESSION['allow_edit']);
    }
    public  function edit($id){
       $client= $this->model->getClient($id);

        $content = $this->view->renderEditView((array)$client);

        echo $this->mainView($content,1);
    }

    public function update($clientID,$lastname,$firstname,$midname,$birthday,$comment_info){
         $this->model->updateClient($clientID,$lastname,$firstname,$midname,$birthday,$comment_info);

         $client= $this->model->getClient($clientID);
         $content = $this->view->renderUpdateView((array)$client);
         echo $this->mainView($content, $_SESSION['allow_edit']);
    }

    public function deleteById($clientID, $delete_comment){
        $this->model->deleteClientbyId($clientID, $delete_comment);
        $client= $this->model->getDeletedClient($clientID);
        $content = $this->view->renderDeleteView((array)$client);
        echo $this->mainView($content, $_SESSION['allow_edit']);
    }
}